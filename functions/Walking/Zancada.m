...
for i=1:length(Timer)-1
    ...
    if t < t_ini
        ...
        if t < t_ini+T_ss
            ...
            t_local=t-t_ini;
            if (Tipo==1 || Tipo==2)
                Mf_x=Cicloidal_1(DF_x/2,T_local,t_local);
            elseif (Tipo==3 || Tipo==4)
                Mf_x=-DF_x/4+Cicloidal_1(DF_x,T_local,t_local);
            else
                Mf_x=-DF_x/4+Cicloidal_1(DF_x/2,T_local,t_local);
            end
            if ((DF_y >=0) && ((Tipo==3) || (Tipo==5))) || (DF_y < 0) && ((Tipo==4) || (Tipo==6))
                Mf_y=E*L_2-DF_y/2+Cicloidal_1(DF_y,T_local,t_local);            
            else
                Mf_y=E*L_2+Cicloidal_1(DF_y,T_local,t_local);
            end
            if mod(TrayWalk.Si+1,2)
            ...
            if (Tipo==1 || Tipo==2) || ((DF_y >= 0) && ((Tipo==4) ) ) || ( DF_y < 0 && Tipo==3)
                Mf_y=DF_y+E*L_2;
            else
                Mf_y=DF_y/2+E*L_2;
            end
    ...
    %P_Lx=Mf_Lx/2;
    if (Tipo==1) || (Tipo == 2)
        %P_x=Cicloidal_3(DF_x/4+D_x_TurnP,T,t);
        P_x=Cicloidal_3(DF_x/4,T,t);
    elseif Tipo==3 || Tipo == 4
        P_x=DF_x/2*(t/T);
    else
    ...
    if (t < T1)
        ...
        if (t < T1+T_ds)
            ...
            if mod(TrayWalk.Si+1,2)
                ...
                P_y=DF_y/2-E*Cicloidal_5(DP_y,T_local,t_local);
            end
...
%Left Leg
PieIzq_Rot_Pel_1=EulYXZ_2_Rot(0,0,XPelvis(6));
PieIzq_Rot_Pel_2=EulYXZ_2_Rot(XPelvis(4),XPelvis(5),0);
...
%Right Leg
PieDer_Rot_Pel_1=EulYXZ_2_Rot(0,0,XPelvis(6));
PieDer_Rot_Pel_2=EulYXZ_2_Rot(XPelvis(4),XPelvis(5),0);
